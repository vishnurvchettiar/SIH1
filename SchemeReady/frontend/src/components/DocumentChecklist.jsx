...
export default function DocumentChecklist({ lang = 'en', profile, setProfile, selectedScheme, onProceedToPack }) {
  ...
  const getStatusLabel = (type) => type === 'verified' ? (t.checklist?.statusVerified || 'Verified') : (t.checklist?.statusMissing || 'Missing');
  ...
  const refreshDocuments = async () => {
    ...
    try {
      ...
      (Array.isArray(rows) ? rows : []).forEach((row) => {
        ...
        if (!config) return;
        grouped[config.id] = { ...row, checklistId: config.id, documentKey: row.documentKey, statusType: 'verified' };
      });
